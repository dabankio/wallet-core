...
import (
	...
	"github.com/lomocoin/HDWallet-Core/core/bch/bchd/txscript"
...
func SignRawTransaction(cmd *btc.SignRawTransactionCmd, chainCfg *chaincfg.Params) (*btcjson.SignRawTransactionResult, error) {
	...
	for _, rti := range cmdInputs {
		...
		amt, err := btcutil.NewAmount(rti.Amount)
		if err != nil {
			return nil, err
		}
		inputValues = append(inputValues, int64(amt.ToUnit(btcutil.AmountSatoshi)))
	}
	...
	// reply.
	signErrs, err := signTransaction(&tx, inputValues, hashType, inputs, keys, scripts, chainCfg)
	if err != nil {
		return nil, err
	}
...
func signTransaction(tx *wire.MsgTx, inputValues []int64, hashType txscript.SigHashType,
	additionalPrevScripts map[wire.OutPoint][]byte, additionalKeysByAddress map[string]*btcutil.WIF,
	p2shRedeemScriptsByAddress map[string][]byte, chainCfg *chaincfg.Params) ([]SignatureError, error) {
	...
	for i, txIn := range tx.TxIn {
		...
		if (hashType&txscript.SigHashSingle) != txscript.SigHashSingle || i < len(tx.TxOut) {
			script, err := txscript.SignTxOutput(chainCfg, tx, i, inputValues[i], prevOutScript,
		...
		vm, err := txscript.NewEngine(prevOutScript, tx, i, txscript.StandardVerifyFlags, nil, nil, inputValues[i])
		if err == nil {
			err = vm.Execute()
		}
		if err != nil {
