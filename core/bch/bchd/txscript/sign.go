...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int, amt int64,
	...
	case PubKeyTy:
		...
		script, err := p2pkSignatureScript(tx, idx, amt, subScript, hashType, key)
		if err != nil {
			return nil, class, nil, 0, err
		}
	...
	case PubKeyHashTy:
		...
		script, err := SignatureScript(tx, idx, amt, subScript, hashType, key, compressed)
		if err != nil {
			return nil, class, nil, 0, err
		}
	...
	case ScriptHashTy:
		...
		return script, class, addresses, nrequired, nil
	case MultiSigTy:
		script, _ := signMultiSig(tx, idx, amt, subScript, hashType, addresses, nrequired, kdb)
		return script, class, addresses, nrequired, nil
	case NullDataTy:
...
func SignTxOutput(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	if class == ScriptHashTy {
		...
		realSigScript, _, _, _, err := sign(chainParams, tx, idx, amt, sigScript, hashType, kdb, sdb)
		if err != nil {
			return nil, err
		}
	...
	return mergeScripts(chainParams, tx, idx, amt, pkScript, class, addresses, nrequired, sigScript, previousScript)
}
